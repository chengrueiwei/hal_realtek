...
static bt_timer_handler_t bt_timer_handler[3];

void mac_RegisterBtTimerHandler(uint32_t tid, bt_timer_handler_t handler)
{
    bt_timer_handler[tid] = handler;
}
...
#if 1
        if (pmac_driver->state_flg.b.rf_rst_pending)
        {
            mac_RstRF();
        }
...
void Zigbee_Handler_Patch(void)
{
    ...
    if (isrsts.b.btcmp0if)
    {
        if (bt_timer_handler[0])
        {
            bt_timer_handler[0]();
        }
    }

    if (isrsts.b.btcmp1if)
    {
        if (bt_timer_handler[1])
        {
            bt_timer_handler[1]();
        }
    }

    if (isrsts.b.btcmp2if)
    {
        if (bt_timer_handler[2])
        {
            bt_timer_handler[2]();
        }
    }
